    ...
    elseif (strlen($pass) < 5 || strlen($pass) > 8)
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if(isset($_POST['chkRememberMe'])){
                setcookie('ckAEmail',$email,time()+ 60*60); //1 hour expiry
                setcookie('ckAPass',$pass,time()+ 60*60); //1 hour expiry
            }
...
<body>
    <h1 align="center">OSP Admin Login Form</h1>
    <hr>
    <form method="POST">
        <!-- Email input -->
        <div class="form-outline mb-4">
            <input type="text" name="txtEmail" class="form-control" required="" value="<?php if(isset($_COOKIE['ckAEmail'])) echo $_COOKIE['ckAEmail']; ?>"/>
            <label class="form-label" for="form2Example1">Email Address</label>
        </div>

        <!-- Password input -->
        <div class="form-outline mb-4">
            <input type="password" name="txtPass" class="form-control" required="" value="<?php if(isset($_COOKIE['ckAPass'])) echo $_COOKIE['ckAPass']; ?>"/>
        ...
        <div class="text-center">
            <p>Not a member? <a href="./AdminReg.php">Register</a></p>
        </div>
    </form>
</body>
